...
class GarmentSegmenter:
    ...
    def _process_image(
        ...
        """
        import os
        # Local server: use real rembg (fast), Render: use mock (slow)
        use_mock = os.getenv("USE_MOCK_SEGMENTATION", "false").lower() == "true"
        ...
        try:
            ...
            if use_mock:
                # Mock mode: Skip rembg, just use the original image
                # This is much faster for demo purposes
                print("Using mock segmentation (skipping rembg)")
                segmented = pil_image.convert("RGBA")
            else:
                # Real mode: Use rembg for proper background removal
                try:
                    from rembg import remove
                    print("Using rembg for real segmentation...")
                    segmented = remove(pil_image)
                except ImportError:
                    print("rembg not available, falling back to mock")
                    segmented = pil_image.convert("RGBA")
                except Exception as e:
                    print(f"rembg failed: {e}, falling back to mock")
                    segmented = pil_image.convert("RGBA")
