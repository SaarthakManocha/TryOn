...
# === Logging Setup ===
def log_step(step: str, status: str = "info", details: str = ""):
    """Print colorful logs for tracking pipeline steps."""
    icons = {
        "start": "🚀",
        "success": "✅",
        "error": "❌",
...
# DISABLED: FAL.AI credits exhausted - using mock mode
...
class CreateBaseBodyRequest(BaseModel):
    ...
    body_build: str
    height_cm: int
    skin_tone_rgb: Optional[List[int]] = None  # Optional, not used yet but sent by app
...
async def extract_face(
    ...
    log_step("FACE EXTRACTION", "start", f"user={user_id}")
    
    if not selfie.content_type or not selfie.content_type.startswith("image/"):
        log_step("FACE EXTRACTION", "error", "Invalid file type")
    ...
    try:
        log_step("FACE EXTRACTION", "upload", f"Saving selfie to {temp_filename}")
        with open(temp_path, "wb") as buffer:
            shutil.copyfileobj(selfie.file, buffer)
        
        log_step("FACE EXTRACTION", "process", "Running MediaPipe face detection...")
        ...
        if result.get("success"):
            skin = result.get("skin_tone_rgb", [])
            log_step("FACE EXTRACTION", "success", f"Skin tone RGB: {skin}")
            if result.get("face_image_url"):
                log_step("FACE EXTRACTION", "upload", "Uploaded to Cloudinary")
        else:
            log_step("FACE EXTRACTION", "error", result.get("error", "Unknown error"))
        
        return FaceExtractionResponse(**result)
        
    except Exception as e:
        log_step("FACE EXTRACTION", "error", str(e))
...
async def api_extract_hair(
    ...
    log_step("HAIR EXTRACTION", "start", f"user={user_id}")
    
    if not selfie.content_type or not selfie.content_type.startswith("image/"):
        log_step("HAIR EXTRACTION", "error", "Invalid file type")
        raise HTTPException(status_code=400, detail="File must be an image")
    ...
    try:
        log_step("HAIR EXTRACTION", "upload", f"Saving selfie to {temp_filename}")
        with open(temp_path, "wb") as buffer:
            shutil.copyfileobj(selfie.file, buffer)
        
        log_step("HAIR EXTRACTION", "process", "Running MediaPipe segmentation...")
        result = extract_hair(
            image_path=temp_path,
            user_id=user_id,
            output_dir=OUTPUT_DIR
        ...
        if result.get("success"):
            log_step("HAIR EXTRACTION", "success", "Hair mask created")
            if result.get("hair_image_url"):
                log_step("HAIR EXTRACTION", "upload", "Uploaded to Cloudinary")
        else:
            log_step("HAIR EXTRACTION", "error", result.get("error", "Unknown error"))
        
        return HairExtractionResponse(**result)
        
    except Exception as e:
        log_step("HAIR EXTRACTION", "error", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def api_create_base_body(request: CreateBaseBodyRequest):
    ...
    log_step("BASE BODY", "start", f"user={request.user_id}, gender={request.gender}")
    
    if request.gender not in ['male', 'female', 'other']:
        log_step("BASE BODY", "error", "Invalid gender")
        raise HTTPException(status_code=400, detail="Invalid gender")
    
    # Map body_build aliases to actual template names
    body_build = request.body_build.lower()
    body_build_map = {
        'average': 'regular',
        'medium': 'regular',
        'slim': 'slim',
    ...
    if body_build not in body_build_map:
        log_step("BASE BODY", "error", f"Invalid body_build: {body_build}")
        raise HTTPException(status_code=400, detail=f"Invalid body_build: {body_build}")
    
    actual_body_build = body_build_map[body_build]
    log_step("BASE BODY", "process", f"build={actual_body_build}, height={request.height_cm}cm")
    
    if not (140 <= request.height_cm <= 210):
        log_step("BASE BODY", "error", "Height out of range")
    ...
    try:
        ...
        if result.get("success"):
            log_step("BASE BODY", "success", "Base body created")
            if result.get("base_body_image_url"):
                log_step("BASE BODY", "upload", "Uploaded to Cloudinary")
        else:
            log_step("BASE BODY", "error", result.get("error", "Unknown error"))
        
        return BaseBodyResponse(**result)
        
    except Exception as e:
        log_step("BASE BODY", "error", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def segment_garment_upload(
    ...
    log_step("GARMENT SEGMENT", "start", f"user={user_id}")
    
    if not image.content_type or not image.content_type.startswith("image/"):
        log_step("GARMENT SEGMENT", "error", "Invalid file type")
        raise HTTPException(status_code=400, detail="File must be an image")
    ...
    try:
        log_step("GARMENT SEGMENT", "upload", f"Saving garment to {temp_filename}")
        with open(temp_path, "wb") as buffer:
            shutil.copyfileobj(image.file, buffer)
        
        log_step("GARMENT SEGMENT", "process", "Running rembg background removal...")
        ...
        if result.get("success"):
            log_step("GARMENT SEGMENT", "success", f"Type: {result.get('garment_type', 'unknown')}")
            if result.get("garment_image_url"):
                log_step("GARMENT SEGMENT", "upload", "Uploaded to Cloudinary")
        else:
            log_step("GARMENT SEGMENT", "error", result.get("error", "Unknown error"))
        
        return GarmentSegmentationResponse(**result)
        
    except Exception as e:
        log_step("GARMENT SEGMENT", "error", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def segment_garment_url(request: SegmentGarmentUrlRequest):
    ...
    log_step("GARMENT SEGMENT URL", "start", f"user={request.user_id}")
    log_step("GARMENT SEGMENT URL", "download", f"Fetching from URL...")
    ...
    try:
        ...
        if result.get("success"):
            log_step("GARMENT SEGMENT URL", "success", f"Type: {result.get('garment_type', 'unknown')}")
        else:
            log_step("GARMENT SEGMENT URL", "error", result.get("error", "Unknown error"))
        
        return GarmentSegmentationResponse(**result)
        
    except Exception as e:
        log_step("GARMENT SEGMENT URL", "error", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def api_tryon(request: TryOnRequest):
    ...
    """
    log_step("TRY-ON", "start", f"user={request.user_id}, type={request.garment_type}")
    
    base_body_path = os.path.join(OUTPUT_DIR, request.user_id, "base_body.png")
    
    if not os.path.exists(base_body_path):
        log_step("TRY-ON", "error", "Base body not found")
        raise HTTPException(
            status_code=400, 
    ...
    if not os.path.exists(request.garment_path):
        log_step("TRY-ON", "error", "Garment image not found")
        raise HTTPException(
            status_code=400, 
    ...
    try:
        log_step("TRY-ON", "process", "Calling IDM-VTON API...")
        ...
        if result.get("success"):
            log_step("TRY-ON", "success", f"Generated: {result.get('tryon_id', 'unknown')}")
            if result.get("tryon_image_url"):
                log_step("TRY-ON", "upload", "Uploaded to Cloudinary")
        else:
            log_step("TRY-ON", "error", result.get("error", "Unknown error"))
        
        return TryOnResponse(**result)
        
    except Exception as e:
        log_step("TRY-ON", "error", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def api_composite(request: CompositeRequest):
    ...
    """
    log_step("COMPOSITE", "start", f"user={request.user_id}")
    
    # Get face image path
    face_path = os.path.join(OUTPUT_DIR, f"{request.user_id}_face.png")
    
    if not os.path.exists(face_path):
        log_step("COMPOSITE", "error", "Face image not found")
        raise HTTPException(
            status_code=400,
    ...
    if not os.path.exists(request.tryon_image_path):
        log_step("COMPOSITE", "error", "Try-on image not found")
        raise HTTPException(
            status_code=400,
    ...
    if len(request.skin_tone_rgb) != 3:
        log_step("COMPOSITE", "error", "Invalid skin tone RGB")
        raise HTTPException(
            status_code=400,
    ...
    if not os.path.exists(hair_path):
        log_step("COMPOSITE", "info", "Hair not found, using face fallback")
        hair_path = None  # Hair is optional, continue without it
    else:
        log_step("COMPOSITE", "info", "Hair image found")
    
    try:
        log_step("COMPOSITE", "process", "Blending face and hair onto body...")
        ...
        if result.get("success"):
            log_step("COMPOSITE", "success", f"Final image: {result.get('final_id', 'unknown')}")
            if result.get("final_image_url"):
                log_step("COMPOSITE", "upload", "Uploaded to Cloudinary")
        else:
            log_step("COMPOSITE", "error", result.get("error", "Unknown error"))
        
        return CompositeResponse(**result)
        
    except Exception as e:
        log_step("COMPOSITE", "error", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def api_search_products(request: ProductSearchRequest):
    ...
    """
    log_step("PRODUCT SEARCH", "start", f"max_results={request.max_results}")
    log_step("PRODUCT SEARCH", "search", f"Searching {request.country.upper()} stores...")
    ...
    try:
        ...
        if result.get("success"):
            ...
            log_step("PRODUCT SEARCH", "success", f"Found {count} products in {time_ms}ms")
        else:
            log_step("PRODUCT SEARCH", "error", result.get("error", "Unknown error"))
        
        return ProductSearchResponse(**result)
        
    except Exception as e:
        log_step("PRODUCT SEARCH", "error", str(e))
