...
class HairExtractor:
    ...
    def extract(self, image_path: str, user_id: str) -> HairExtractionResult:
        ...
        try:
            ...
            # Save hair mask
            print(f"Saving hair mask for user {user_id}...")
            hair_mask_path = self._save_hair_mask(hair_mask, user_id)
            print(f"Hair mask saved to: {hair_mask_path}")
            
            # Save hair image (with transparency)
            print(f"Saving hair image...")
            hair_image_path = self._save_hair_image(image_cv, hair_mask, user_id)
            print(f"Hair image saved to: {hair_image_path}")
            
            # Upload to Cloud Storage
            print(f"Uploading hair mask to Cloudinary...")
            hair_mask_url = StorageService.upload_hair_mask(user_id, hair_mask_path)
            print(f"Hair mask URL: {hair_mask_url}")
            
            print(f"Uploading hair image to Cloudinary...")
            hair_image_url = StorageService.upload_hair(user_id, hair_image_path)
            print(f"Hair image URL: {hair_image_url}")
    ...
    def _save_hair_image(
        ...
        """Save hair image with transparency (RGBA), resized to fit Cloudinary limits."""
        h, w = image.shape[:2]
        
        # Resize if too large (Cloudinary has 10MB limit)
        max_dimension = 1024
